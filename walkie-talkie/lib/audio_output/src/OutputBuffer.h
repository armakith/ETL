  ...
  void remove_samples(int16_t *samples, int count)
  {
    ...
    for (int i = 0; i < count; i++)
    {
      ...
      if (m_buffering && m_available_samples < m_number_samples_to_buffer)
      {
        ...
        Serial.println("Available Samples:");
        Serial.println(m_available_samples);
        Serial.println("Number of Samples " + m_number_samples_to_buffer);
        samples[i] = 0;
      }
      else
      {
    ...
    Serial.println("Reached Semaphore");
    xSemaphoreGive(m_semaphore);
  }
};
